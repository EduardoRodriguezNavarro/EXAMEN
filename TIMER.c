...
extern void Configurar_Timer0B(void(*task)(void), unsigned long int periodo)
{
    ...
    TIMER0->TAMR = (0x3<<0); //configurar el modo del timer (Periodic Timer Mode) p. 732
    TIMER0->TAMR = (1<<4); //para count-down
    //0->count-down
    ...
    NVIC->IP[9] = (NVIC->IP[4]&0xFFFFFF00) | 0x00000020; //para habilitar las interrupciones del timer
...
extern void Timer0B_Handler(void) //cuando ocurra la interrupción se apunta en esta dirección
{
