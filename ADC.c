...
extern void Configura_Reg_ADC0(void)
{
    ...
    SYSCTL->RCGCGPIO |= (0<<5)|(1<<4)|(0<<3)|(0<<2)|(0<<1)|(0<<0);//PUERTOS DONDE HABILITAMOS EL RELOJ
    
    //| (0<<2) | (0<<5)
    //    PE2     PE5
    GPIOE_AHB->DIR =(0<<1) ; //HABIITAMOS PINES COMO ENTRADAS
    //                PE1      
    //| (1<<2) | (1<<5)
    GPIOE_AHB->AFSEL =  (1<<1) ;//SE ACTIVA FUNCIÓN ANALOGICA
   // | (0<<2) | (0<<5 )
    GPIOE_AHB->DEN = (0<<1) ;//APAGAMOS DIGITAL MODE P.781
    GPIOE_AHB->PCTL = GPIOE_AHB->PCTL & (0xFFFFFF0F);
    //| (1<<2) | (1<<5)
    GPIOE_AHB->AMSEL =(1<<1) ;//ACTIVAMOS ANALOG MODE P.786
